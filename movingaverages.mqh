                          ...
                          const int period, const double &price[], double &buffer[])
{
   ...
   if (prev_calculated == 0)
   {
      ...
      limit = prev_calculated - 1;
   //--- main loop
   for (i = limit, j = 0; i < rates_total && j < rates_total - prev_calculated && !IsStopped(); i++, j++)

      // TODO check if price correspondance is correct or if it is picking the last value of the current open bar instead of the last closed bar
      buffer[i] = price[prev_calculated == 0 ? i : j] * dSmoothFactor + buffer[i - 1] * (1.0 - dSmoothFactor);
   //--- restore as_series flags
   if (as_series_price)
      ArraySetAsSeries(price, true);
   if (as_series_buffer)
